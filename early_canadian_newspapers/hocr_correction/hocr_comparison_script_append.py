...
def run_evaluation(orig_path, manual_path, ai_path, output_csv="MASTER_id_based_evaluation.csv"):
    ...
    # saving to csv file (append mode, write header only if file is new or empty)
    import os
    if detailed_results:
        ...
        print(f"\n✅ Detailed log appended to: {output_csv}")
    else:
...
if __name__ == "__main__":
    IMAGE = "006_01_1"
    ORIGINAL_HOCR = f"comparisons-v2/approach-1d/hocr_original/{IMAGE}.hocr"
    MANUAL_HOCR = f"comparisons-v2/approach-1d/hocr_manually_corrected/{IMAGE}_Corrected_OCR.hocr"
    AI_HOCR = f"comparisons-v2/approach-1d/hocr_gemini_corrected/{IMAGE}/3_flash_medium_AI_corrected.hocR"
